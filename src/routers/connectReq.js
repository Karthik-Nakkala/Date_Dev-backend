...
connectReqRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      if (status === "interested" && toUserExists.emailId) {
        try {
          const subject = `${req.user.firstName} wants to connect with you!`;
          const bodyHtml = `<h1>Connection Request</h1><p>Hi ${toUserExists.firstName},</p><p><strong>${req.user.firstName}</strong> is interested in connecting with you on DateDev!</p>`;
          const bodyText = `Hi ${toUserExists.firstName}, ${req.user.firstName} is interested in connecting with you on DateDev!`;
          const emailRes = await sendEmail.run(
            toUserExists.emailId,
            subject,
            bodyHtml,
            bodyText,
          );
          ...
          //needed to think some alternative
        }
      }

      res.send(
        `${req.user.firstName} interested in connecting with ${toUserExists.firstName}`,
      );
    } catch (err) {
